...
func TestParseProvinceCity(t *testing.T) {
	...
	_, p, c := parseProvinceCity(cfg, path)
	if p != "浙江省" || c != "杭州市" {
...
func TestDeduplicateExactHash(t *testing.T) {
	...
	kept, dups := deduplicate(files, 0.8, 0.2)
	if len(kept) != 2 {
...
func TestShouldFallbackToOriginalDir(t *testing.T) {
	...
	if !shouldFallbackToOriginalDir(cfg, VideoFile{Province: "浙江省", City: "未知城市"}) {
		t.Fatalf("未知城市应回落原目录")
	}
	if shouldFallbackToOriginalDir(cfg, VideoFile{Province: "浙江省", City: "杭州市"}) {
...
func TestOrganizeNonVideoFilesKeepRelativePath(t *testing.T) {
	...
	records, errs := organizeNonVideoFiles(cfg, []string{src})
	if len(errs) != 0 {
